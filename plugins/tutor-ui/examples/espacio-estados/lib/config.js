...
/* Tolera BOM: Notepad y `Out-File -Encoding utf8` de PowerShell lo agregan, y
   JSON.parse se rompe con él — el config parecería inexistente. */
function readJSON(p) {
  try {
...
/* Si stdin se cierra (EOF) a mitad del wizard, rl.question() nunca resuelve y el
   proceso se muere en silencio con código 0. Cortamos eso: si el readline cierra,
   devolvemos el default en vez de colgarnos para siempre. */
function ask(rl, question, fallback) {
  return new Promise(function (resolve) {
    ...
    rl.once("close", function () { done(""); });
    rl.question(question).then(done, function () { done(""); });
  });
}

const NO_TTY_HELP =
  "No puedo hacer las preguntas de configuración porque la entrada no es una terminal\n" +
  "     interactiva. Configuralo de una de estas dos formas y volvé a arrancar:\n" +
  "\n" +
  "       a) por entorno:   TUTOR_PROVIDER=cli   (o =api con ANTHROPIC_API_KEY)\n" +
  "       b) por archivo:   escribí " + "{\"provider\":\"cli\"}" + " en\n" +
  "                         " + path.join(os.homedir(), ".tutor-ui", "config.json");

async function runWizard(o) {
  o = o || {};
  const target = o.configPath || configPath();
  let rl = o.rl;
  if (!rl) {
    if (!process.stdin.isTTY) {
      const e = new Error("wizard needs a TTY");
      e.userMessage = NO_TTY_HELP;
      throw e;
    }
  ...
  if (choice === "2") {
    ...
    const hasEnv = !!((o.env || process.env)[envName]);
    out("");
    if (hasEnv) {
      ...
      if (paste) {
        cfg.api.key = paste;
        out("");
        out("   ⚠️  La key queda guardada en " + target + " (permisos 0600).");
